...
// .wrangler/tmp/bundle-B0jkiX/checked-fetch.js
...
// .wrangler/tmp/pages-cms9pb/functionsWorker-0.018419146802312714.mjs
...
var getNow = /* @__PURE__ */ __name2(() => (/* @__PURE__ */ new Date()).toISOString(), "getNow");
var genId = /* @__PURE__ */ __name2(() => crypto.randomUUID(), "genId");
...
var crud = /* @__PURE__ */ __name2((path, table, getSort = "createdAt DESC") => {
  ...
  app.post(`/${path}`, async (c) => {
    ...
    await c.env.DB.prepare(`INSERT INTO ${table} (${keys.join(",")}) VALUES (${qMarks})`).bind(...values).run();
  ...
  app.put(`/${path}/:id`, async (c) => {
    ...
    await c.env.DB.prepare(`UPDATE ${table} SET ${sets} WHERE id = ?`).bind(...values, id).run();
...
app.put("/settings", async (c) => {
  ...
  const { results } = await c.env.DB.prepare("SELECT * FROM settings WHERE id = ?").bind("global_settings").all();
  if (results.length > 0) {
    await c.env.DB.prepare("UPDATE settings SET min_sar_rate = ?, min_aed_rate = ?, updatedAt = ? WHERE id = ?").bind(body.min_sar_rate, body.min_aed_rate, getNow(), "global_settings").run();
  } else {
    await c.env.DB.prepare("INSERT INTO settings (id, createdAt, updatedAt, min_sar_rate, min_aed_rate) VALUES (?, ?, ?, ?, ?)").bind("global_settings", getNow(), getNow(), body.min_sar_rate, body.min_aed_rate).run();
  }
  return c.json(true);
});
...
// .wrangler/tmp/bundle-B0jkiX/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-B0jkiX/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.018419146802312714.js.map
