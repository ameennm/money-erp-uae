...
var crud = /* @__PURE__ */ __name((path, table, getSort = "createdAt DESC") => {
  ...
  app.put(`/${path}/:id`, async (c) => {
    ...
    const validCols = VALID_COLUMNS[table] || [];
    const updates = Object.keys(body).filter((key) => validCols.includes(key)).map((key) => `${key} = ?`).join(", ");
    if (!updates) return c.json({ success: true, message: "No valid columns to update" });
    const values = Object.keys(body).filter((key) => validCols.includes(key)).map((key) => body[key]);
...
// ../.wrangler/tmp/pages-5VrdXw/functionsRoutes-0.8340128201683759.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
